...
class ConteudoSubtopico(BaseModel):
    ...
    simulador_interativo_recomendado: Optional[str] = Field(
        default=None, 
        description="RELEVÂNCIA ESTRITA: Proponha um simulador interativo APENAS se for altamente relevante e pedagógico para o conceito do subtópico (ex: análise de resíduos, correlação, estatística descritiva, convergência do TLC, detecção de outliers). NÃO gere gráficos/simuladores irrelevantes apenas para ter um gráfico. Se não for necessário ou pedagógico, deixe estritamente como None."
    )
...
class QuestaoFechada(BaseModel):
    ...
    codigo_plotly: Optional[str] = Field(
        default=None,
        description="RELEVÂNCIA ESTRITA: Código Python puro utilizando Plotly para gerar um gráfico interativo de suporte para esta questão (ex: curvas de distribuição, dispersão, boxplot) APENAS se for realmente relevante e útil para a resolução (ex: resíduos, correlação, descritivas, etc.). Não gere gráficos irrelevantes apenas para ter uma imagem. O código deve criar um objeto de figura chamado 'fig'. Não inclua importações, utilize a paleta do professor e use chaves únicas. Se não for necessário gráfico, deixe None."
    )
...
class QuestaoAberta(BaseModel):
    ...
    codigo_plotly: Optional[str] = Field(
        default=None,
        description="RELEVÂNCIA ESTRITA: Código Python puro utilizando Plotly para gerar um gráfico interativo de suporte para esta questão (ex: densidades, distribuições) APENAS se for altamente relevante para a resolução ou visualização do problema (ex: resíduos, correlação, descritivas). Não gere gráficos irrelevantes. O código deve criar um objeto de figura chamado 'fig'. Não inclua importações, utilize a paleta do professor. Se não for necessário gráfico, deixe None."
    )
    referencia_livro: Optional[str] = Field(
        default=None,
...
class PaginaLapidada(BaseModel):
    ...
    simulador_interativo_recomendado: Optional[str] = Field(
        default=None,
        description="Descrição detalhada do simulador interativo recomendado para esta página se e somente se for altamente relevante e aplicável."
    )
...
class AulaUnificadaELapidada(BaseModel):
    ...
    simuladores_da_aula: List[MapeamentoSimulador] = Field(
        description="Lista contendo o mapeamento de onde os gráficos devem entrar. Atribua gráficos apenas onde for vital, útil e pedagogicamente relevante (ex: resíduos, correlação, estatísticas descritivas), garantindo que nenhum gráfico seja igual ao outro. Se nenhum for relevante, deixe a lista vazia."
    )
