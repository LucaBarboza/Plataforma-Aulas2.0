...
def run_page():
    ...
    # 1. Faz a varredura dinâmica na pasta /logdasaulasgeradas ordenando por data (mais recente primeiro)
    if os.path.exists("logdasaulasgeradas"):
        arquivos_json = [f for f in os.listdir("logdasaulasgeradas") if f.endswith(".log.json")]
        # Ordena os arquivos por data de modificação física (mais recente no topo)
        arquivos_json.sort(key=lambda f: os.path.getmtime(os.path.join("logdasaulasgeradas", f)), reverse=True)
        ...
        for f in arquivos_json:
            ...
            try:
                with open(caminho_log, "r", encoding="utf-8") as lf:
                    ...
                    data_fmt = f" [{timestamp}]" if timestamp else ""
                    label_bonito = f"📊 {num_str}: {tema} ({disciplina}){data_fmt}"
    ...
    if opcoes_log:
        ...
        # Seleciona automaticamente o relatório mais recente (índice 1)
        log_selecionado_label = st.selectbox("Selecione o Relatório de Auditoria:", options=lista_opcoes, index=1 if len(lista_opcoes) > 1 else 0)
                ...
                if not subtopicos_log:
                    ...
                    for i, sub in enumerate(subtopicos_log):
                        ...
                        with st.expander(header_label):
                            ...
                            with col_sub2:
                                max_tentativas_sub = sub.get("max_tentativas", 3)
                                st.write(f"**Tentativas do Escritor:** {tentativas_sub} de {max_tentativas_sub}")
