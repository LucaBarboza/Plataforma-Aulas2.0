...
def programar_fatia_teoria(dados_subtopico: dict, nome_simulador: str, descricao_simulador: str = "", motor_grafico: str = "plotly", chave_suffix: str = "", cor_principal: str = "#1E3A8A", cor_critica: str = "#991B1B", cor_secundaria: str = "#10B981", cor_alerta: str = "#F59E0B") -> str:
    ...
    4. FORMALISMO MATEMÁTICO E PERFEIÇÃO LATEX (KATEX COMPATIBLE): 
       ...
       - ATENÇÃO: NUNCA inclua cifrões (`$` ou `$$`) dentro da string passada ao `st.latex(r"...")` (ex: faça `st.latex(r"\\theta_1 = 0.5")` e NUNCA `st.latex(r"$$\\theta_1 = 0.5$$")`).
       - Toda chamada `st.latex()` e `st.markdown()` DEVE usar obrigatoriamente o prefixo raw string (`r"..."`).
       - Qualquer palavra de texto comum dentro de blocos LaTeX DEVE estar contida no comando `\\text{{...}}` (ex: `\\hat{{\\beta}}_1 \\text{{ onde }} x`).
    ...
    6. FIDELIDADE À TERMINOLOGIA E VOCABULÁRIO DO PROFESSOR: Toda a prosa explicativa, rótulos de variáveis, legendas e textos dos cartões DEVEM utilizar rigorosamente o vocabulário e a nomenclatura do professor fornecida no JSON (ex: se o professor utiliza 'Valor Esperado' em vez de 'Esperança', mantenha 'Valor Esperado' em toda a interface).
    ...
    4. GRÁFICOS NO MEIO DO TEXTO: Se o parâmetro 'nome_simulador' for fornecido (não vazio), insira os widgets de controle, sliders e o gráfico correspondente a esse conceito (utilizando a biblioteca '{motor_grafico}') imediatamente após o bloco de formalismo matemático (`st.info` / `st.latex`).
        O simulador a ser programado é: '{nome_simulador}'.
        As diretrizes de funcionamento do simulador e especificações de parâmetros são:
        ---
        {descricao_simulador}
        ---
        IMPORTAÇÕES PROIBIDAS: É TERMINANTEMENTE PROIBIDO gerar imports de 'matplotlib', 'seaborn', 'bokeh' ou bibliotecas não declaradas. Use exclusivamente Plotly (plotly.graph_objects/plotly.express), Streamlit, NumPy e Pandas.
        ---
        DIRETRIZES OBRIGATÓRIAS DE DESIGN DE INTERATIVIDADE PREMIUM:
        - Divida as variáveis e seletores de controle em colunas elegantes usando `st.columns` para economizar espaço e evitar que fiquem gigantes na tela.
        - Combine múltiplos tipos de controle do Streamlit: `st.slider`, `st.selectbox`, `st.number_input` de forma apropriada e elegante.
        - Sempre que houver um parâmetro booleano ou visualizações opcionais (ex: "Exibir Reta de Regressão", "Mostrar Resíduos", "Sombrear Região Crítica", etc.), use o componente `st.toggle()` do Streamlit para manter a interface limpa e moderna.
        - Se for conceitualmente apropriado para demonstrar o conceito em questão (como no ajuste de retas de regressão ou cálculo de estatísticas amostrais), crie uma pequena tabela interativa usando `st.data_editor` preenchida com dados iniciais de amostra. Isso permite que o aluno altere os dados diretamente e veja os resultados estatísticos e a curva de plotagem se recalcularem em tempo real no gráfico!
        - **MANDATÓRIO (LAUDO TEXTUAL DINÂMICO):** Logo abaixo do gráfico do simulador, insira um painel explicativo dinâmico (`st.info` ou `st.markdown`) usando f-strings Python reativas baseadas nos valores selecionados nos sliders/widgets pelo aluno (Ex: f"Com nível de significância de {{alfa}}... a região crítica é {{regiao_critica}} e a decisão é Rejeitar H0" ou f"Com tamanho de amostra n = {{n}}, o erro padrão cai para {{erro_padrao:.4f}}, estreitando a curva..."). Este laudo dinâmico deve explicar didaticamente o que está ocorrendo sob as interações do aluno.
...
def programar_fatia_exercicios(dados_exercicios: dict) -> str:
    ...
    REGRAS DE INTERATIVIDADE, GAMIFICAÇÃO E SEGURANÇA CONTRA KEYERRORS:
    1. Inicialize e gerencie o progresso e score gamificado do aluno usando `st.session_state`:
       - Crie no topo da aba de exercícios um dicionário `st.session_state.respostas_certas` se não existir:
         `if "respostas_certas" not in st.session_state: st.session_state.respostas_certas = {}`
       - Calcule e exiba uma barra de progresso estilizada (`st.progress`) e o placar de acertos/estudos no topo da aba.
         O total de exercícios a serem resolvidos deve ser calculado dinamicamente:
         `total_exercicios = len(dados_exercicios.get("questoes_multipla_escolha", [])) + len(dados_exercicios.get("questoes_discursivas", []))`
         `acertos = sum(1 for k, v in st.session_state.respostas_certas.items() if v)`
         Se `total_exercicios > 0`, exiba a barra `st.progress(acertos / total_exercicios)` e um `st.markdown(f"🏆 **Seu Progresso no Caderno:** {acertos} de {total_exercicios} desafios concluídos!")`.
    2. Crie uma interface interativa que itera dinamicamente sobre a lista 'dados_exercicios["questoes_multipla_escolha"]'.
       ...
       - Se a questão contiver um campo 'codigo_plotly' e ele estiver preenchido (não nulo/vazio), execute esse código dinamicamente de forma segura dentro de um bloco try-except (usando exec(codigo, globals(), local_vars)) e exiba o gráfico resultante usando `st.plotly_chart(local_vars["fig"], use_container_width=True, key=f"plotly_chart_mcq_{i}")` se 'fig' for criada.
       ...
       - Se a questão contiver um campo 'referencia_livro' preenchido (não nulo/vazio), exiba-o de forma legível logo após o enunciado ou dica usando o ícone do livro e itálico: `st.markdown(f"📖 *Referência: {referencia_livro}*")` de forma a creditar o livro/material do RAG.
       - Crie um botão de verificação: `if st.button("✅ Verificar Resposta", key=f"btn_mcq_{i}"):`. Ao ser clicado, compare a alternativa marcada pelo aluno com a correta. Se correta: exiba `st.success("Correto! Muito bem.")`, atualize `st.session_state.respostas_certas[f"mcq_{i}"] = True` e chame `st.rerun()`. Se incorreta: exiba `st.error("Resposta incorreta. Tente novamente!")`, defina `st.session_state.respostas_certas[f"mcq_{i}"] = False` e chame `st.rerun()`.
    ...
    3. Crie uma interface para iterar sobre a lista 'dados_exercicios["questoes_discursivas"]'.
       - Crie uma seção contendo o enunciado, referência e um campo de resposta de texto `st.text_area("Sua resposta (Prosa):", key=f"text_disc_{i}")`.
       - Se a questão contiver um campo 'codigo_plotly' e ele estiver preenchido (não nulo/vazio), execute esse código dinamicamente de forma segura dentro de um bloco try-except (usando exec(codigo, globals(), local_vars)) e exiba o gráfico resultante usando `st.plotly_chart(local_vars["fig"], use_container_width=True, key=f"plotly_chart_disc_{i}")` se 'fig' for criada.
       - Se a questão contiver o campo 'resposta_numerica_esperada' e ele não for nulo/vazio (ou seja, exige cálculo numérico específico):
         - Exiba um campo numérico `st.number_input("Digite o resultado numérico calculado para validação:", key=f"num_disc_{i}")` e um botão "Validar Cálculo", com chave exclusiva baseada em i.
         - Ao clicar em "Validar Cálculo", verifique se o valor absoluto da diferença entre a resposta do usuário e a resposta esperada está dentro de uma tolerância razoável (por exemplo, 1% de margem ou diferença absoluta menor que 0.01: `abs(user_val - esperada) <= max(0.01, 0.01 * abs(esperada))`).
         - Se validado com sucesso: exiba `st.success("Resposta Numérica Correta! Excelente trabalho de cálculo.")`, defina `st.session_state.respostas_certas[f"disc_{i}"] = True` e chame `st.rerun()`.
         - Se incorreto: exiba `st.error("O valor calculado difere do gabarito oficial. Verifique seus arredondamentos e fórmulas e tente novamente.")`, defina `st.session_state.respostas_certas[f"disc_{i}"] = False` e chame `st.rerun()`.
       - Se a questão NÃO contiver o campo 'resposta_numerica_esperada' (sendo qualitativa):
         - Forneça um botão ou checkbox `st.checkbox("Marque aqui após estudar e responder esta questão", key=f"check_disc_{i}")` para fins de progresso do caderno. Ao marcar, defina `st.session_state.respostas_certas[f"disc_{i}"] = True` e chame `st.rerun()`. Ao desmarcar, defina `st.session_state.respostas_certas[f"disc_{i}"] = False` e chame `st.rerun()`.
       - Renderize a dica de forma segura com `.get("dica", "Dica indisponível")` num botão de dica.
       - Se a questão contiver um campo 'referencia_livro' preenchido (não nulo/vazio), exiba-o de forma legível: `st.markdown(f"📖 *Referência: {referencia_livro}*")` para creditar a fonte do RAG.
    ...
    4. NÃO inclua importações globais ou redefinição de variáveis globais. O código deve iniciar no recuo de 0 espaços.
