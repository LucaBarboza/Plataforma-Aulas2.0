...
4. Avaliação de Grounding (Metadados do RAG): Se o campo 'fontes_rag' contiver itens, exija obrigatoriamente que cada fonte especifique o Nome do Livro/Slide/Material ('livro_autor') e a Seção ou Capítulo ('capitulo'). Se o nome do material ou a seção/capítulo estiverem em branco, nulos ou genéricos (ex: 'N/A' ou 'Fonte não mapeada'), REPROVE o bloco (`aprovado = False`). Caso o campo 'fontes_rag' seja uma lista VAZIA `[]` (devido à ausência de materiais RAG para a aula), o bloco É TOTALMENTE VÁLIDO e NÃO deve ser reprovado por isso. PROIBIDO exigir ou forçar fontes fictícias fora do RAG.
